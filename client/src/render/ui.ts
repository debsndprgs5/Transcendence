...
function sideColor(side: Side): string {
  switch (side) {
    case 'left':   return '#6EB4FF';
    case 'right':  return '#FFD278';
    case 'top':    return '#78F7A8';
    case 'bottom': return '#FF8CA0';
  }
}

// Babylon GUI Grid wants numbers + isPixel flag (false = star sizing)
...
function avatarCircle(col: string, size = 34): GUI.Ellipse {
  const e = new GUI.Ellipse();
  e.width = `${size}px`;
  e.height = `${size}px`;
  e.thickness = 0;
  e.background = col;
...
function label(text: string, size = 16, color = '#fff', weight: string = 'normal', hpx?: number) {
  ...
  t.fontSize = size;
  t.fontWeight = weight;
  t.height = `${hpx ?? Math.round(size * 1.6)}px`; // px, not %
  ...
  t.resizeToFit = false;       // width is provided by parent
...
function makeChip(side: Side, username: string) {
  ...
  container.width  = '100%';                 // fill the grid cell
  container.thickness = 0;
  // Slightly lighter than the strip so it’s readable
  container.background = 'rgba(255,255,255,0.06)';
  container.cornerRadius = 12;
  container.horizontalAlignment = GUI.Control.HORIZONTAL_ALIGNMENT_STRETCH;
  container.verticalAlignment   = GUI.Control.VERTICAL_ALIGNMENT_CENTER;

  // avatar col (48px) + flexible text col
  const grid = makeGrid([{ px: 48 }, { star: 1 }]);
  grid.horizontalAlignment = GUI.Control.HORIZONTAL_ALIGNMENT_STRETCH;
  grid.verticalAlignment   = GUI.Control.VERTICAL_ALIGNMENT_STRETCH;
  container.addControl(grid);

  const avatar = avatarCircle(sideColor(side));
  grid.addControl(avatar, 0, 0);

  const stack = new GUI.StackPanel();
  stack.isVertical = true;
  stack.paddingLeft = '10px';
  stack.width = '100%';                      // <-- important so text gets space
  ...
  return { container, avatarHolder: avatar, name, score };
}
...
export function updateHUD(
  ...
  // Time
  ui.timeText.text = `${mmss(elapsedSeconds)} 's`;
