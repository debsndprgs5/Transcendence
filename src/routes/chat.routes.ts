...
export async function chatRoutes(fastify:FastifyInstance){
   //Route -> POST api/chat 
    ...
    const {username, message, chatroom} = request.body as {username:string, message:string, chatroom:number}
    const user = await userManagment.getUserByName(username)
    chatManagment.createMessage(user.our_index, chatroom, message)
    if(room == 0) //chat general
       await SendGeneralMessage(message, user)
    else
        await SendChatRoomMessage(chatroom, message, user)
    
    //Route -> GET api/chat/history
    fastify.get('/chat/history')
    //Middleware check here to add 
    const {uname, chatroom} = request.body as {uname:string, chatroom:number}
    const user = await userManagment.getUserByName(uname)
    if(!user)
        error(500)("Wierd internal error , you are hacking man ?!")
    const fullhistory = await chatManagment.getMessagesByChatRoom(chatroom, 50)
    const userhistory = await chatManagment.getMessagesByUserInChatRoom(user.our_index, chatroom)
