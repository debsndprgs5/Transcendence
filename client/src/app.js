				...
				btn.addEventListener('click', async (e) => {
					...
					if (confirm("Delete this room ?")) {
						try {
							...
							showNotification({ message: 'Error duling delete.', type: 'error', duration: 5000 });
						}
					}
				});
			});
...
function showNotification({
  message = '',
  type = 'info',       // success, error, info, warning, prompt
  duration = 3000,     // ignored if type === 'prompt'
  placeholder = '',    // only for prompt
  onConfirm = null,    // callback(value) for prompt
  onCancel = null,     // callback() for prompt
}) {
  if (type === 'prompt') {
    // Prompt mode, ask for an input and setup buttons
    ...
    return; // prompt dont create normal notif, special behavior
  }

  // Otherwise, basic notif
