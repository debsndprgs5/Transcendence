...
// 		bottom: undefined
// 	};
// 	private paddleRoots: Partial<Record<'left'|'right'|'top'|'bottom', BABYLON.TransformNode>> = {};
...
//   const dist   = Math.max(width, depth) * 1.3; // how far back
//   const height = 50;

//   // starting position from player's side
//   let pos = new BABYLON.Vector3(0, height, -dist);
//   switch (this.playerSide) {
//     case 'left':   pos = new BABYLON.Vector3(-dist, height, -3); break;
//     case 'right':  pos = new BABYLON.Vector3(+dist, height, -3); break;
//     case 'top':    pos = new BABYLON.Vector3(-3, height, +dist); break;
//     case 'bottom': pos = new BABYLON.Vector3(-3, height, -dist); break;
//   }

//   this.camera = new BABYLON.FreeCamera("camera", pos, this.scene);
//   this.camera.minZ = 0.1;
//   this.camera.maxZ = 5000;
//   this.camera.inertia = 0.7;
//   this.camera.speed = 0.75;

//   // Don’t steal A/D; mouse can still move camera
//   //this.camera.keysUp = this.camera.keysDown = this.camera.keysLeft = this.camera.keysRight = [];

//   this.camera.setTarget(new BABYLON.Vector3(0, 0, 0));
//   this.camera.attachControl(this.engine.getRenderingCanvas(), true);

//   // Clamp & keep facing center every frame
//   this.scene.onBeforeRenderObservable.add(() => this.clampAndOrientCamera());
// }

// private clampAndOrientCamera() {
//   const width = this.playerCount === 2 ? LIMIT.arenaWidth2p  : LIMIT.arenaWidth4p;
//   const depth = this.playerCount === 2 ? LIMIT.arenaLength2p : LIMIT.arenaLength4p;

//   const margin = 25;  // how far beyond arena you allow roaming
...
// 		// Setup each side
// 		const sides = [
// 			{ name: 'top',    origin: new BABYLON.Vector3(-halfW, -3, +halfD), length: width,  axis: 'x', dir: +1, yawOff: Math.PI,        target: this.frontWalls },
// 			{ name: 'bottom', origin: new BABYLON.Vector3(+halfW, -3, -halfD), length: width,  axis: 'x', dir: -1, yawOff: 0,              target: this.frontWalls },
// 			{ name: 'left',   origin: new BABYLON.Vector3(-halfW, -3, -halfD), length: depth,  axis: 'z', dir: +1, yawOff: Math.PI/2,    target: this.sideWalls  },
// 			{ name: 'right',  origin: new BABYLON.Vector3(+halfW, -3, +halfD), length: depth,  axis: 'z', dir: -1, yawOff:-Math.PI/2,    target: this.sideWalls  }
// 		];
...
//   this.moveBottomToY(ballRoot, -2.5);
...
//     if (mat instanceof BABYLON.PBRMaterial) { mat.emissiveColor = glow; mat.emissiveIntensity = 1.5; }
...
//   this.ballObj.light.intensity = (0.6 + 0.8 * t)*2;
...
// // Return TransformNodes (safer than Mesh for grouping)
// private loadPaddleTemplates(
//   path: string,
//   file: string
...
//     BABYLON.SceneLoader.ImportMesh("", path, file, this.scene,
//       (meshes) => {
//         // Hide everything that came in
//         meshes.forEach(m => m.setEnabled(false));

//         // Our two template roots
//         const lightRoot = new BABYLON.TransformNode("paddle_light_root", this.scene);
//         const darkRoot  = new BABYLON.TransformNode("paddle_dark_root",  this.scene);
//         lightRoot.setEnabled(false);
//         darkRoot.setEnabled(false);

//         // 1) Try exact top-node names :
//         //    Box_low_0  (light)
//         //    Box_low001_1 (dark)
//         let lightTop = this.scene.getNodeByName("Box_low_0") as BABYLON.TransformNode | null;
//         let darkTop  = this.scene.getNodeByName("Box_low001_1") as BABYLON.TransformNode | null;

//         // 2) Fallback: find by MATERIAL NAME (Box_light / Box_dark), then climb to topmost ancestor
...
//             mesh: m.name,
//             material: (m as BABYLON.Mesh).material?.name || "(none)"
//           })));
//           return reject("Couldn’t locate both variants (Box_low_0 / Box_low001_1 or materials Box_light / Box_dark).");
//         }

//         // Re-parent those variants under our hidden template roots
//         lightTop.parent = lightRoot;
//         darkTop.parent  = darkRoot;

//         resolve({ light: lightRoot, dark: darkRoot });
//       },
//       undefined,
//       (_s, msg) => reject(msg)
//     );
...
//     const { light, dark } = await this.loadPaddleTemplates("../assets/", "sci-fi_box.glb");
...
//     this.moveBottomToY(root, -3);

//     const meshes = root.getChildMeshes() as BABYLON.Mesh[];
//     for (const m of meshes) this.glow?.addExcludedMesh?.(m);

//     const lightColor = isPlayer
//       ? new BABYLON.Color3(0.20, 1.00, 0.40)
//       : new BABYLON.Color3(0.00, 0.85, 1.00);
...
//     // Choose a stable "main" mesh (largest volume) rather than meshes[0]
//     let main: BABYLON.Mesh | undefined;
...
//     if (main) {
//       this.paddles.push(main);
//       this.paddleMap[side] = main;
//     }
// 	this.paddleRoots[side] = root; 
//   }
// }
...
// 			if (k === 'a') this.inputState.left  = true;
// 			if (k === 'd') this.inputState.right = true;
// 		}
// 		this.handleKeyUp = (e: KeyboardEvent) => {
// 			const k = e.key.toLowerCase();
// 			if (k === 'a') this.inputState.left  = false;
// 			if (k === 'd') this.inputState.right = false;
// 		};
// 		window.addEventListener('keydown', this.handleKeyDown);
// 		window.addEventListener('keyup', this.handleKeyUp);
// 		// drive input each frame
// 		this.scene.onBeforeRenderObservable.add(() => this.processInput());
// 	}
...
// 			this.inputState.left === this.inputState.right ? 'stop'
// 			: this.inputState.left ? 'left' : 'right';

// 		// Invert controls for RIGHT and BOTTOM sides
// 		if ((this.playerSide === 'right' || this.playerSide === 'bottom') && dir !== 'stop') {
...
// 				const root = this.paddleRoots[side];
// 				if (!root) return;
// 				if (side === 'left' || side === 'right') {
// 				root.position.z = pos;    // server sends Y for H paddles -> our Z
// 				} else {
// 				root.position.x = pos;    // server sends X for V paddles -> our X
// 				}
// 			});

// 			// Ball (keep your kinetic roll)
// 			const firstBall = Object.values(update.balls)[0];
// 			if (firstBall) this.applyBallKinetics(new BABYLON.Vector3(firstBall.x, 0, firstBall.y));
...
import { setupGUI, setupPauseUI } from './ui';
