...
async function verifyAndExtractUser(ws: WebSocket, request: any): Promise<{ userId: number, fullUser: user } | null> {
...
export async function setupMessageHandlers(ws: WebSocket, player: players) {
	ws.on('message', async (data: RawData) => {
		console.log('GAME : Message received from client:', data.toString());
		let parsed: any;
		try {
